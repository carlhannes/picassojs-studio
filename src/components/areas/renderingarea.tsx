...
// @ts-expect-error
import picasso from 'picasso.js/packages/picasso.js/src/index';
// @ts-expect-error
import picQ from 'picasso.js/plugins/q/src/index';
// @ts-expect-error
import picHammer from 'picasso.js/plugins/hammer/src/index';

import { useResize } from '../../core/hooks';

import styles from './renderingarea.module.scss';
...
export type RenderingAreaProps = {
  title: string;
  code: string;
  data: string | any;
  api: string;
...
function RenderingArea({ title, code, data, api, settings, dataSource }: RenderingAreaProps) {
  const element = React.useRef(null);
  const [chart, setChart] = React.useState<any>();

  const [message, setMessage] = React.useState('Loading...');

  const prevComposition = React.useRef<any>();
  const prevData = React.useRef<string | any>();
  ...
  React.useEffect(() => {
    ...
    if (chart && chart.update && typeof code === 'string' && (typeof data === 'string' || typeof data === 'object')) {
      ...
      const dataHashId: string | number = typeof data === 'string' ? data : hashFromString(JSON.stringify(data));
  ...
  // render
  return (
    <Box display="flex" flexDirection="column" flexGrow={1} className={styles.root}>
      <div ref={element} style={{ display: 'flex', flexGrow: 1 }} className={styles.render} />
