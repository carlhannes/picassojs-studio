...
class HypercubeGenerator {
   ...
   */
  randomNullInsert(arr, chance = 0.1, randomizer = Math.random) {
    return arr.map(v => randomizer() <= chance ? null : v);
  }
   ...
   */
  generateRandomData(dimensions, measures, rows, sorted = false, randomizer = Math.random) {
    return [
      [
      ...
      ...this.random2dArr(
        ...
        (x, y, width, height) => (!sorted ? (x / width * y / height) : 1) * randomizer,
      ),
    ];
  }
